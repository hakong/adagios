...
%changelog
* Sun Feb 07 2016 hakon.gislason <hakon.gislason@gmail.com>
- Update adagios.spec (hakon.gislason@gmail.com)
- Update Makefile (hakon.gislason@gmail.com)
- Update adagios (hakon.gislason@gmail.com)
- Update __init__.py (hakon.gislason@gmail.com)
- Update adagios.spec (hakon.gislason@gmail.com)
- Update Makefile (hakon.gislason@gmail.com)
- Update adagios (hakon.gislason@gmail.com)
- Update __init__.py (hakon.gislason@gmail.com)
- Fix hosts with no services appearing as down. (palli@opensource.is)
- Properly handle unicode text when sending email. (palli@opensource.is)
- Centralize reload_configfile code in settings (nathan@acceleration.net)
- Better sudoers fail documentation (tommi@plainvanillagames.com)
- Default to using nagios_service (tommi@plainvanillagames.com)
- Enable editing nagios_service within adagios (tommi@plainvanillagames.com)
- RPM linting (tommi@plainvanillagames.com)
- Default to python-django, special case for rhel6
  (tommi@plainvanillagames.com)
- Better error detection for sudoers problems (tommi@plainvanillagames.com)
- Remove empty line with spaces (tommi@plainvanillagames.com)
- Remove old un-used class ADaemon (tommi@plainvanillagames.com)
- Remove unused variables (tommi@plainvanillagames.com)
- Refactor calls to FakeAdagiosEnvironment() (palli@opensource.is)
- objectbrowser/add: Include data from all fields (palli@opensource.is)
- rest/tests.py: Run unit-tests in isolation. (palli@opensource.is)
- Add unit tests for rest reschedule calls (tommi@plainvanillagames.com)
- Add mock as a requirement for development (tommi@plainvanillagames.com)
- Always do int conversion on check_time (tommi@plainvanillagames.com)
- Remove unnecessary int conversion (tommi@plainvanillagames.com)
- Use url() in urls.py and remove settings (tommi@plainvanillagames.com)
- Enable bi.urls by default (tommi@plainvanillagames.com)
- Fix converting None to int (tommi@plainvanillagames.com)
- Replace obsolete mimetype with content_type (tommi@plainvanillagames.com)
- Fix travis and lack of adagios.conf (palli@opensource.is)
- Fix advanced search escaping search text (palli@opensource.is)
- Revert "Travis: Explicitly copy adagios.conf" (palli@opensource.is)
- Fix missing description of copied objects (palli@opensource.is)
- Travis: Explicitly copy adagios.conf (palli@opensource.is)
- Update rest.py (brian-github2@starvingprogrammer.com)
- Fix status view snippets_log : missing parameter (tim@clerc.im)
- Handle IOError exception when object is saved (root@shinken)
- Handle IOError exception when object is saved (tim@clerc.im)
- Changed misleading error message (hakon.gislason@gmail.com)
- Fix status/dashboard/ showing wrong data (palli@opensource.is)
- Update bulk_edit.html (dbizsley@googlemail.com)
- Mail objectlists should link to problems page (palli@opensource.is)
- Make log access configurable (palli@opensource.is)
- status/rest: Use utils.get_log_entries internally (palli@opensource.is)
- Funnel all calls to LogFiles>utils.get_log_entries (palli@opensource.is)
- Limit per page hosts/services in status interface (palli@opensource.is)
- Fix timed reload on service pages (palli@opensource.is)
- service_detail page javascript errors on spaces (palli@opensource.is)
- Fix delete of saved searches with spaces (palli@opensource.is)
- Fix bad merge which removed advanced search (tommi@plainvanillagames.com)
- Fix missing modals on some views (palli@opensource.is)
- Fix typo in adagios_status (palli@opensource.is)
- setup.py should not overwrite adagios.conf (palli@opensource.is)
- Test subclassing pynag instead of mapping functions
  (tommi@plainvanillagames.com)
- Update sudoers for systemctl and nagios group usage
  (tommi@plainvanillagames.com)
- /bi/: Deleting process should remove references (palli@opensource.is)
- Add script build_branches.sh (palli@opensource.is)
- Remove extra print statement (palli@opensource.is)
- Fix Custom SSI typo (palli@opensource.is)
- New endpoint: /objectbrowser/import (palli@opensource.is)
- Fix typo in status/perfdata2 (palli@opensource.is)
- Explicitly update globals in context processors (palli@opensource.is)
- Serverside includes for emails (palli@opensource.is)
- New feature: Saved searches (palli@opensource.is)
- Make unhandled hosts/services configurable (palli@opensource.is)
- Advanced Search Dialog for adagios (palli@opensource.is)
- Refactor and code clean up in adagios.status.utils (palli@opensource.is)
- Fix adagios-logo-32.png path (hakon.gislason@gmail.com)
- Code cleanup - Refactor utils.get_services() (palli@opensource.is)
- Fixed copy and pasted comments (pmorgan.sa@gmail.com)
- Allow unicode characters to exist in adagios.conf (palli@opensource.is)
- objectbrowser/forms.py: Minor cleanup (palli@opensource.is)
- Fix: Unicode errors in objectbrowser forms (palli@opensource.is)
- status/state_history: Remove obsolete services (palli@opensource.is)
- objectbrowser/add: Fix Implicit generic-host (palli@opensource.is)
- status/detail: Fix line-height for check_command (palli@opensource.is)
- Add Adagios icon in top navbar (thibault.cohen@savoirfairelinux.com)
- Add Adagios images (thibault.cohen@savoirfairelinux.com)
- Fixed typo in views.py (pmorgan.sa@gmail.com)
- Fix to display contact members in status_contactgroup.html template
  (pmorgan.sa@gmail.com)
- Updated status_contactugroup.html to fix contactgroup members listing
  (pmorgan.sa@gmail.com)
- Make selenium tests skip if there is an error (tommi@tommi.org)
- Initialize fake adagios env after selenium (tommi@tommi.org)
- Remove unused imports and move imports to top (tommi@tommi.org)
- Fix stdout/err and add status (tommi@tommi.org)
- Skip selenium for pull requests from forks (tommi@tommi.org)
- Invalid variable name access_keys, fixed (tommi@tommi.org)
- adagios.daemon in bi and misc.forms (tommi@tommi.org)
- Selenium rework (tommi@tommi.org)
- Remove chosen javascript (palli@opensource.is)
- SeleniumTestCase moved, add multiple webdrivers (tommi@tommi.org)
- Remove pynag.Control and add adagios.daemon (tomas@plainvanillagames.com)
- Call quit instead of close (tommi@tommi.org)
- Look for more than zero, failed on 1 host (tommi@tommi.org)
- Make driver only exit once (tommi@tommi.org)
- Set hub_url to saucelabs (tommi@tommi.org)
- More work on travis saucelabs integration (tommi@tommi.org)
- Test hooking up travis-ci with saucelabs (tommi@tommi.org)
- Use same selenium session across unit test files (tommi@tommi.org)
- Allow testserver to run on multiple ports (tommi@tommi.org)
- Add missing import of By (tommi@tommi.org)
- Add selenium tests for objectbrowser (tommi@tommi.org)
- Only run LiveServerTestCase if selenium is there (tommi@tommi.org)
- Move SeleniumTestCase to utils.py, re-usability (tommi@tommi.org)
- setup.py: update dependencies (ginermail@gmail.com)
- Clean packaging (titilambert@gmail.com)
- Skip selenium tests properly when not installed (tommi@tommi.org)
- Move away from splinter to selenium (tommi@tommi.org)
- Disable reschedule button in status_detail if active checks are not enabled
  (pmorgan.sa@gmail.com)
- est for too many top_alert_producers (tommi@tommi.org)
- Support for arguments to splinter (tommi@tommi.org)
- Splinter based browser testing added (tommi@tommi.org)
- STATIC_URL needed for LiveServerTestCase (tommi@tommi.org)
